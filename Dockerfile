...
RUN R -e 'install.packages("remotes")' && R -e 'remotes::install_github("neo4j-rstats/neo4r@dev")'
...
CMD cd /home/neo4j && sudo chmod -R 777 neo4j-community-* && cd neo4j-community-* && sudo bin/neo4j-admin set-initial-password ${NEOPASS} && sudo bin/neo4j start && /init 